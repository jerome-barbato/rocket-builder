        ...
        self.__construct = function ($map, config, callback) {
            ...
            if (!googlemap_init) {
                ...
                if (document.readyState == 'complete') {
                    $('head')
                        .append('<script src="https://maps.google.com/maps/api/js?key=' + google_api_key + '&callback=googlemap_init"></script>');
                }
                else {
        ...
        self.init = function ($map, callback) {
            ...
            self.context.gmap = $map.gmap3(self.config.map).then(function (result) {
                ...
                google.maps.event.addListener(self.context.map, 'zoom_changed', function () {
                    ...
                    self.context.$map.trigger('map.zoom', [
                        self.context.map,
                        zoomLevel
                    ]);
                });
        ...
        self.addMarkers = function (markers, fit, icon) {
            ...
            self.context.gmap.on({
                ...
                mouseover: function (marker) {
                    ...
                    self.context.$map.trigger('map.over', [
                        self.context.map,
                        marker.index
                    ]);
                },
                mouseout : function (marker) {
                    ...
                    if (!self.context.has_overlay) {
                        ...
                        self.context.$map.trigger('map.out', [
                            self.context.map,
                            marker.index
                        ]);
                    }
                ...
                click    : function (marker) {
                    ...
                    self.context.$map.trigger('map.click', [
                        self.context.map,
                        marker.index
                    ]);
